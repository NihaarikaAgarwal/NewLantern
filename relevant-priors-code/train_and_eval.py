...
import joblib
from app.encoder import embed_texts
...
def train(public_json_path: str = "relevant_priors_public.json", test_size: float = 0.2, random_state: int = 42):
    ...
    # save artifacts (trained on training fold)
    joblib.dump(clf, Path("app") / "classifier.joblib")
    joblib.dump(scaler, Path("app") / "scaler.joblib")

    # evaluate on train
    probs_train = clf.predict_proba(Xs_train)[:, 1]
    preds_train = (probs_train >= 0.5).astype(int)
    correct_train = (preds_train == y_train).sum()
    incorrect_train = (preds_train != y_train).sum()
    acc_train = correct_train / (correct_train + incorrect_train)

    # evaluate on holdout
    probs_hold = clf.predict_proba(Xs_hold)[:, 1]
    preds_hold = (probs_hold >= 0.5).astype(int)
    correct_hold = (preds_hold == y_hold).sum()
    incorrect_hold = (preds_hold != y_hold).sum()
    acc_hold = correct_hold / (correct_hold + incorrect_hold)

    print(f"Trained on {len(y_train)} pairs — train accuracy: {acc_train:.4f} (correct={correct_train} incorrect={incorrect_train})")
